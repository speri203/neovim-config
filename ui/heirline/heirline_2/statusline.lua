...
local myconditions = require "user.ui.heirline.conditions"
local sep = require "user.ui.heirline.separators"
...
M.Mode = {
  ...
  {
    provider = function(self) return string.format(" %s ", self.mode_icons[self.mode]) end,
    hl = function(self)
...
M.Branch = {
  condition = function() return git_branch() ~= nil end,
  {
    provider = function() return string.format("  %s", git_branch()) end,
    hl = { fg = "green", bg = "gray" },
  },
  {
    provider = sep.rounded_right,
...
M.FilePath = {
  init = function(self) self.bufname = vim.api.nvim_buf_get_name(0) end,
  hl = { bg = "bg_statusline" },
  provider = " ",
  {
    condition = function(self) return require("my.configure.heirline.conditions").should_show_filename(self.bufname) end,
    provider = function()
      local buf = 0
      local mdpreview_session = vim.b.mdpreview_session ---@diagnostic disable-line
      if mdpreview_session then buf = mdpreview_session.source_buf end
...
M.UnsavedChanges = {
  init = function(self) self.unsaved_count = unsaved_count() end,
  {
    condition = function(self) return self.unsaved_count > 0 end,
    {
      {
...
M.OnePassword = {
  ...
  {
    provider = function()
      if not vim.g.op_nvim_remote_loaded then return " 1P: Signed Out" end
...
M.LspFormatToggle = {
  ...
  hl = { bg = "bg_statusline" },
  on_click = {
    callback = function() require("my.lsp.utils").toggle_formatting_enabled() end,
    name = "heirline_LSP",
  },
  {
  ...
  },
  {
    provider = function()
      ...
      if name then return string.format(" (%s)  ", name) end
      return "  "
    end,
    hl = { bg = "bg_statusline" },
  },
