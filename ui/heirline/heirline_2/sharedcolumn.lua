...
M.DiagnosticSign = {
  {
    condition = function() return not conditions.lsp_attached or #vim.diagnostic.get(0) == 0 end,
    provider = " ",
  },
  {
    condition = function() return conditions.lsp_attached or #vim.diagnostic.get(0) > 0 end,
    init = function(self)
      self.sign = nil
      self.has_sign = false
    ...
    provider = function(self)
      if self.has_sign then return require("user.ui.heirline.icons")[self.sign.name] end
    end,
    hl = function(self)
...
M.LineNumAndGitIndicator = {
  ...
  provider = function() return string.format("%%=%s", vim.v.lnum) end,
  hl = {
    fg = "gray",
  },
  {
    ...
    provider = " ▏",
    hl = function(self)
      if self.has_sign then return self.hlgroup end
    end,
  },
}
