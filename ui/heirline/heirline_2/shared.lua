...
function M.FileIcon(bg_color)
  return {
    ...
    hl = { bg = bg_color },
    {
      ...
      hl = function(self) return { fg = utils.get_highlight(self.icon_hl).fg, bg = bg_color } end,
    },
  }
end
...
M.FilePath = {
  ...
  condition = function(self) return self.bufname ~= "" and not vim.startswith(self.bufname, "component://") end,
  provider = function(self) return Path.relative(self.bufname) end,
}

local icons = require "user.ui.heirline.icons"
...
local diagnostics_base = {
  init = function(self)
    ...
    for key, severity in pairs(severities) do
      self[key] = #vim.tbl_filter(function(d) return d.severity == severity end, all_diagnostics)
    end
  end,
}
...
function M.Diagnostics(is_winbar, bg)
  ...
  return utils.insert(
    ...
    unpack(vim.tbl_map(function(severity)
      local component = {
        provider = function(self) return string.format("%s%s ", icons[severity], self[severity]) end,
        hl = function()
          return { fg = utils.get_highlight(string.format("DiagnosticSign%s", severity_hl[severity])).fg, bg = bg }
        end,
      }
      if is_winbar then component.condition = function(self) return self[severity] > 0 end end
      return component
