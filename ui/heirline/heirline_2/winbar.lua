...
M.UniqueFilename = {
  init = function(self) self.bufname = get_unique_filename(vim.api.nvim_buf_get_name(0)) end,
  {
    condition = function(self) return require("user.ui.heirline.conditions").should_show_filename(self.bufname) end,
    provider = function(self) return string.format(" %s ", self.bufname) end,
    hl = { bg = "black" },
  },
  {
    -- file save status indicator
    condition = function() return vim.bo.modified == true end,
    provider = " ",
    hl = { bg = "black" },
  },
  {
...
M.Diagnostics = {
  ...
  require("user.ui.heirline.shared").Diagnostics(true, "gray"),
  {
    provider = sep.rounded_right,
...
M.Navic = {
  condition = function() return conditions.lsp_attached() and require("nvim-navic").is_available() end,
  provider = function() return string.format(" %s", require("nvim-navic").get_location()) end,
  hl = { bg = "bg_statusline" },
  {
    provider = "%<",
  },
